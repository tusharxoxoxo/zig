...
fn make(step: *Step, prog_node: *std.Progress.Node) !void {
    ...
    const tmp_dir_path = blk: {
        const rand_int = std.crypto.random.int(u64);
        const tmp_dir_sub_path = "tmp" ++ fs.path.sep_str ++ std.Build.hex64(rand_int);
        break :blk tmp_dir_sub_path;
    };
    ...
    try b.cache_root.handle.makePath(o_sub_path);
    try b.cache_root.handle.rename(tmp_dir_path, o_sub_path);
